                                ...
                                <div key={index} className="p-2 bg-slate-50 rounded-lg border border-slate-200 relative">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
                                        ...
                                        <div>
                                            ...
                                            <input
                                                value={`₹ ${product.product_price || '0.00'}`}
                                                readOnly
                                                className="w-full px-2 py-1.5 text-xs rounded-md border border-slate-300 bg-white text-slate-600 font-semibold"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-semibold text-slate-700 mb-1">
                ...
                <div className="bg-white rounded-3xl shadow-2xl border border-slate-200">
                    <div className="p-6 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-slate-100">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                            ...
                            <button
                                onClick={exportToExcel}
                                className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-xl font-semibold shadow-lg"
                            >
                                <TfiExport size={18} />
                                Export Excel
                            </button>
                        ...
                        <div className="flex flex-col md:flex-row gap-3">
                            ...
                            <select
                                value={filterType}
                                onChange={(e) => setFilterType(e.target.value)}
                                className="px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-indigo-500 outline-none transition-all bg-white"
                            >
                                <option value="all">All Fields</option>
                                <option value="name">Customer Name</option>
                                <option value="account">Account Number</option>
                        ...
                        {loading ? (
                            ...
                            <table className="w-full">
                                <thead className="bg-gradient-to-r from-slate-100 to-slate-200 sticky top-0 z-10">
                                    <tr>
                                        {['#', 'Account', 'Customer', 'Product', 'Category', 'Price', 'Qty', 'Total', 'Date', 'Type', 'Actions'].map((header) => (
                                            <th key={header} className="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase">
                                                {header}
                                            </th>
                                        ))}
                                    </tr>
                                    ...
                                    {filteredProducts.length > 0 ? (
                                        filteredProducts.map((item, i) => (
                                            <tr key={item.id} className="hover:bg-indigo-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-sm text-slate-600">{item.date}</td>
                                                <td className="px-6 py-4">
                                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${item.transaction_type === 'cash' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'
                                                        }`}>
                                                        {item.transaction_type === 'cash' ? '💵 Cash' : '📋 Borrow'}
                                                    </span>
                                                </td>
