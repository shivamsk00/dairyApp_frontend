...
import { FaPen, FaCheck, FaTimes, FaWallet, FaMoneyBillWave, FaCalendarAlt, FaUser, FaReceipt, FaSync } from 'react-icons/fa'
...
const CashEntryPage = () => {
  ...
  const handleRefresh = async () => {
    try {
      ...
      CustomToast.success('Page refreshed successfully')
    } catch (error) {
      console.error('Refresh error:', error)
      CustomToast.error('Error refreshing page')
    } finally {
      setRefreshing(false)
    }
  }
  ...
  const filteredEntries = allEntries.filter((entry) => {
    ...
    // Date matching (Normalize entry date to YYYY-MM-DD)
    let matchDate = true;
    if (filters.date) {
      ...
      if (entry.date) {
        if (entry.date.includes('-')) {
          ...
          if (parts[0].length === 2) { // DD-MM-YYYY
            entryDateISO = `${parts[2]}-${parts[1]}-${parts[0]}`;
          }
  ...
  const statistics = {
    totalReceived: filteredEntries
      .filter(e => e.credit_debit_mode === 'received')
      .reduce((sum, e) => sum + parseFloat(e.amount || 0), 0),
    totalGiven: filteredEntries
  ...
  const handleEditClick = async (entry) => {
    ...
    console.log('Original date:', entry.date, 'Formatted date:', formattedDate)
  }
              ...
              <div className="p-8 border-b border-slate-200">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-sm text-slate-600">
                    Total: <span className="font-bold text-indigo-600">{filteredEntries.length}</span> / {allEntries.length} entries
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 border-b border-slate-200 bg-slate-50 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none text-sm transition-all shadow-sm"
                  />
                </div>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none text-sm relative transition-all shadow-sm"
                  />
                  {/* Clear Date Button */}
                  {filters.date && (
                    <button
                      onClick={() => setFilters(prev => ({ ...prev, date: '' }))}
                      className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-red-500 p-1 rounded-full hover:bg-slate-200 transition-colors"
